...
#define ANY_VECTOR Vector<N, T>
#define ANY_VECTOR_DEC_PARAM template <size_t N, typename T> 

ANY_VECTOR_DEC_PARAM
inline ANY_VECTOR operator + (const ANY_VECTOR& a) {
	return a;
}
...
ANY_VECTOR_DEC_PARAM
inline bool operator == (const ANY_VECTOR& a, const ANY_VECTOR& b) {
	for (size_t i = 0; i < N; i++) {
		if (a[i] != b[i])
			return false;
	}
...
ANY_VECTOR_DEC_PARAM
inline ANY_VECTOR& operator += (ANY_VECTOR& a, const ANY_VECTOR& b) {
	for (size_t i = 0; i < N; i++) {
		a[i] += b[i];
	}
	return a;
...
ANY_VECTOR_DEC_PARAM
inline ANY_VECTOR& operator *= (ANY_VECTOR& a, T b) {
	for (size_t i = 0; i < N; i++) {
		a[i] *= b;
	}
	return a;
...
inline bool vector_near(const Vector<N, T>& a, const Vector<N, T>& b, T dist) {
	return vector_length_sqare(a - b) <= dist;
}

// 判断两个float Vector是否近似相似
template <size_t N>
inline bool vector_near_equal(const Vector<N, float>& a, const Vector<N, float>& b, float e = 0.0001) {
	return vector_near(a, b, e);
}

// 判断两个double Vector是否近似相似
template <size_t N>
inline bool vector_near_equal(const Vector<N, double>& a, const Vector<N, double>& b, float e = 0.0000001) {
	return vector_near(a, b, e);
}
...
template <size_t ROW, size_t COL, typename T> struct Matrix {
	...
	inline void SetCol(size_t col, const Vector<COL, T>& a) {
		assert(col < COL);
	...
	inline Matrix<ROW - 1, COL - 1, T> GetMinor(size_t row, size_t col) const {
		...
		for (size_t r = 0; r < ROW - 1; r++) {
			for (size_t c = 0; c < COL - 1; c++) {
				ret[r][c] = m[r < row ? r : r + 1][c < col ? c : c + 1];
			}
		}
		return ret;
	}
...
inline static uint32_t vector_to_color(const Vec4f& color) {
	uint32_t r = Between(0, 255, (int)(color.r * 255.0f));
	uint32_t g = Between(0, 255, (int)(color.g * 255.0f));
	uint32_t b = Between(0, 255, (int)(color.b * 255.0f));
	uint32_t a = Between(0, 255, (int)(color.a * 255.0f));
...
//---------------------------------------------------------------------
// Bitmap 类，用于读写图片，画点画线读取颜色
//---------------------------------------------------------------------
class Bitmap {
public:
	inline virtual ~Bitmap() { if (_bits) delete[]_bits; _bits = nullptr; }
...
// ShaderContext，由 VS 设置，再由渲染器按像素逐点插值之后，供 PS 读取
struct ShaderContext {
	template <typename K, typename V>
	using map = std::map<K, V>;
	map<int, int> a;
	
	map<int, float> varying_float;
	map<int, Vec2f> varying_vec2f;
	map<int, Vec3f> varying_vec3f;
	map<int, Vec4f> varying_vec4f;
};

// VertextShader
typedef std::function<Vec4f(int index, ShaderContext& ouput)> VertexShader;
...
public:
	inline void Reset() {
		...
		if (_frame_buffer) delete _frame_buffer; _frame_buffer = nullptr;
		if (_depth_buffer) {
	...
	inline bool DrawPrimitive() {
		if (_frame_buffer == nullptr || _vertex_shader == nullptr) return false;
		// 顶点初始化
		for (int k = 0; k < 3; k++) {
